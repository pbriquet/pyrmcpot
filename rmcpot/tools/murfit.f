...
C     Modified (input/output streamlined, comments added) 
C     from  WIEN97, SRC_eosfit/
C     by A. Postnikov,  postnikov@univ-metz.fr
C     Slightly modified by R. G. A. Veiga (Universidade 
C     Federal do ABC) to be used with RMCPOT
C

      implicit none
      integer length,mdata
      parameter (length=256, mdata=401)
      character*256  string                                 
      real*8 AA,BB,CC,bfac, bmod,ene,third,
     .       x(4),x12,x23,x31,y(mdata),dx,diag(mdata),
     .       epsfcn,factor,fjac(mdata,4),ftol,
     .       qtf(4),vol,wa1(4),wa2(4),wa3(4),wa4(mdata),xmax,xmin,
     .       xx,ee,press,entha
      integer i,iflag,info,imid,ipvt(4),j,ldfjac,m,maxfev,mode,
     .        n,nfev,nprint,nrep
      common /vol/ vol(mdata)
      common /ene/ ene(mdata)
      external eos2,murna,murna1
      data third/0.3333333333333333d0/
      data bfac/160.21766208/
      open (55,file='enevol.dat',status='old',form='formatted',err=11)
      goto 12
   11 continue
      print *,'   Error opening file!'
      stop
   12 continue
C --- read in data till end of file:
C 1    read(55,101,end=2) string
C      if (string(1:1).eq.'#') then
C        goto 1     !  ignore lines beginning with  #
C      else
C        read(string(1:length),*,err=3) vol(i),ene(i)
C      endif

      read(55,*) nrep

      i=1
 1    read(55,*,end=2) vol(i),ene(i)    

C      vol(i)=vol(i)/(0.529177**3) ! if input volume is in Ang**3
C      ene(i)=ene(i)/13.61         ! if input energy is in eV
...
C     three-point parabolic fit: E = AA*vol**2 + BB*vol + CC
     ...
     -       ene(imid)/x12/x23 - 
     -       ene(m)   /x31/x23
      BB =   ene(1)   *(vol(imid)+vol(m)   )/x12/x31 +
     +       ene(imid)*(vol(1)   +vol(m)   )/x12/x23 +
     +       ene(m)   *(vol(1)   +vol(imid))/x31/x23 
      CC = - ene(1)   *vol(imid)*vol(m)   /x12/x31 -
     -       ene(imid)*vol(1)   *vol(m)   /x12/x23 -
     -       ene(m)   *vol(1)   *vol(imid)/x31/x23 
C 
C     initial guess of Murnaghan parameters from 2d ortder E(V) fit:
      x(1) = CC - 0.25d0*BB*BB/AA  !  min. energy
...
C     
      implicit none
      integer mdata,m,n,i,iflag
      parameter (mdata=401)
      real*8 x(*),fvec(*),vol,ene,xh
      common /vol/ vol(mdata)
      common /ene/ ene(mdata)
      if (n.eq.3) then
       xh=x(4)
      ...
      do i=1,m
        fvec(i)=x(1)+
     +          x(3)*vol(i)/(x(4)*(x(4)-1.d0))*(x(4)*
     *          (1.d0-x(2)/vol(i))+(x(2)/vol(i))**x(4)-1.d0) - ene(i)
      enddo
      if (n.eq.3) then
       x(4)=xh
      endif
...
C------------------------------------------------------------------
C
      subroutine murna1(x,vol,ene,press,entha)
C     fit to the Murnaghan equation of state.
C
C     On fitting, the parameters x(1)...x(4) are:
...
C     
      implicit none
      real*8 x(4),vol,ene,press,entha
C --- fit to energy:
        ene = x(1)+
     +        x(3)*vol/(x(4)*(x(4)-1.d0))*
     *       (x(4)*(1.d0-x(2)/vol)+(x(2)/vol)**x(4) - 1.d0) 
C --- fit to pressure:
        press = x(3)/x(4)*((x(2)/vol)**x(4) - 1.d0)
C --- enthalpy:
	entha = ene + press*vol
C     The pressure was so far in Ry/a.u.^3; convert into GPa:
	press = press * 14714.0d0
