...
! Input variables
	...
	logical,public		:: eam_existemb(MAXELEEAM)	! If .true., read the embedding data from the files provided
	logical,public		:: eam_existrho(MAXELEEAM+2)	! If .true., read the density data from the files provided
	logical,public		:: eam_existpair(MAXELEEAM)	! If .true., read the elemental pair data from the files provided
	...
	namelist /inpeamalloy/ eam_nel,eam_el,eam_existemb,eam_existrho,eam_pairfile,eam_rhofile,eam_embfile,eam_nr,eam_cd_ncoeff, &
	& eam_nrho,eam_dr,eam_drho,eam_anumber,eam_amass,eam_a0,eam_rmax,eam_struct,eam_rhofunc,eam_pairfunc,eam_existcrosspair, &
	& eam_existpair,eam_cut,eam_nknotspair,eam_nknotsrho,eam_embfunc,eam_r0,eam_rmin,eam_fs,eam_fs_rhoscale,eam_knotsrho,eam_knotspair
...
contains
	...
	subroutine eam_readinput
		...
		eam_rhofile(3)='rho_21.dat'
		eam_rhofile(4)='rho_12.dat'
...
418		read(24,inpeamalloy)
		...
		elseif(eam_nel==2)then
			...
			if(eam_existemb(1).and.eam_existemb(2))then
				nparabsent=nparabsent+nparemb*2
			elseif(eam_existemb(1).or.eam_existemb(2))then
				nparabsent=nparabsent+nparemb
			end if

			if(eam_existrho(1).and.eam_existrho(2))then
				nparabsent=nparabsent+nparrho*2
			elseif(eam_existrho(1).or.eam_existrho(2))then
				nparabsent=nparabsent+nparrho
			end if

			if(eam_fs.and.(.not.eam_fs_rhoscale))then
				if(eam_existrho(3).and.eam_existrho(4))then
					nparabsent=nparabsent+nparrho*2
				elseif(eam_existrho(3).or.eam_existrho(4))then
					nparabsent=nparabsent+nparrho
				end if
			end if
			...
			! Populate the additional density for Finnis-Sinclair with data from preexisting potential
			if(eam_fs.and.(.not.eam_fs_rhoscale))then
				if(eam_existrho(3))then
					open(57,file=eam_rhofile(3),status='old')

					do i=1,eam_nr
						read(57,*) dummy,eam_rho(3,i) 
					end do

					close(57)
				end if

				if(eam_existrho(4))then
					open(58,file=eam_rhofile(4),status='old')

					do i=1,eam_nr
						read(58,*) dummy,eam_rho(4,i) 
					end do
	...
	function eam_writepot(potfile,isthebest)		
		...
		if(isthebest)then
			...
			if(eam_nel==2)then	
				...
				if(eam_fs)then
					if(eam_fs_rhoscale.or.(.not.eam_existrho(3)))then
						open(64,file=eam_rhofile(3))
					end if

					if(eam_fs_rhoscale.or.(.not.eam_existrho(4)))then
		...
		do i=1,eam_nr
			...
			if(isthebest.and.(.not.eam_existrho(1)))then
				r=(i-1)*eam_dr
		...
		if(eam_nel==2)then
			! If Finnis-Sinclair, density function of the 2-1 interaction
			if(eam_fs)then
				do i=1,eam_nr
					write(48,*) eam_rho(3,i)

					if(isthebest.and.(.not.eam_existrho(3)))then
						r=(i-1)*eam_dr
			...
			! If Finnis-Sinclair, density function for the 1-2 interaction
			if(eam_fs)then
				do i=1,eam_nr		
					write(48,*) eam_rho(4,i)

					if(isthebest.and.(.not.eam_existrho(4)))then
						r=(i-1)*eam_dr
			...
			do i=1,eam_nr		
				...
				if(isthebest.and.(.not.eam_existrho(2)))then
					r=(i-1)*eam_dr
